...
SET category_id = (SELECT MIN(id)
                   ...
                   WHERE fc2.name = (SELECT name
                                     FROM file_categories fc3
                                     WHERE fc3.id = drive_entries.category_id))
WHERE category_id NOT IN (SELECT MIN(id)
...
UPDATE file_entries
SET drive_id = (SELECT MIN(de2.id)
                FROM drive_entries de2
                         INNER JOIN drive_entries de3 ON de3.id = file_entries.drive_id
                WHERE de2.name = de3.name
                  AND de2.category_id = de3.category_id)
WHERE drive_id NOT IN (SELECT MIN(id)
                       FROM drive_entries

use chrono::Local;
use lister::domain::model::file_entry::FileEntry;
use lister::domain::model::language::Language;
use lister::infrastructure::database::pool::SqliteRepositoryPool;
use lister::infrastructure::database::repository::ListerRepository;
use lister::infrastructure::i18n::json_translation_loader::load_translations;
use lister::utils::dialogs::popup_error_and_exit;
use std::collections::HashSet;
use std::time::Instant;
use tempfile::TempDir;

// Test helpers and fixtures
struct TestFixture {
    _temp_dir: TempDir, // Store it to prevent its disposal
    repository: ListerRepository,
}

impl TestFixture {
    fn new() -> Self {
        let temp_dir = TempDir::new().expect("Failed to create temp directory");
        let db_path = temp_dir.path().join("test.db");
        let db_url = format!("sqlite://{}", db_path.display());

        let pool =
            SqliteRepositoryPool::new(&db_url).unwrap_or_else(|error| popup_error_and_exit(error));

        Self {
            _temp_dir: temp_dir,
            repository: ListerRepository::new(pool),
        }
    }

    fn create_test_files() -> Vec<FileEntry> {
        vec![
            FileEntry {
                path: "documents/report.pdf".to_string(),
                size_bytes: 1024,
            },
            FileEntry {
                path: "images/photo.jpg".to_string(),
                size_bytes: 2048,
            },
            FileEntry {
                path: "code/main.rs".to_string(),
                size_bytes: 512,
            },
            FileEntry {
                path: "documents/invoice.pdf".to_string(),
                size_bytes: 768,
            },
        ]
    }
}

#[test]
fn test_complete_file_indexing_workflow() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Test indexing workflow
    let result = fixture.repository.save("Work", "Laptop", 1024, &files);

    assert!(result.is_ok());
    assert_eq!(result.unwrap(), 4);

    // Verify files were indexed
    let query_result = fixture.repository.search_files_paginated(None, None, 0, 10);
    assert!(query_result.is_ok());

    let actual = query_result.unwrap();
    assert_eq!(actual.len(), 4);

    // Verify file metadata
    let first_file = &actual[0];
    assert_eq!(first_file.category_name, "Work");
    assert_eq!(first_file.drive_name, "Laptop");
    assert_eq!(first_file.drive_available_space, 1024);
    assert!(first_file.drive_insertion_time <= Local::now().naive_local());
    assert!(files.iter().any(|f| f.path == first_file.path));
}

#[test]
fn test_duplicate_removal_workflow() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index files first time
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("First indexing failed");

    // Remove duplicates
    let remove_result = fixture.repository.remove_duplicates("Work", "Laptop");
    assert!(remove_result.is_ok());

    // Verify files were removed
    let query_result = fixture
        .repository
        .search_files_paginated(None, None, 0, 10)
        .unwrap();
    assert_eq!(query_result.len(), 0);

    // Index again after removal
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Second indexing failed");

    let final_result = fixture
        .repository
        .search_files_paginated(None, None, 0, 10)
        .unwrap();
    assert_eq!(final_result.len(), 4);
}

#[test]
fn test_file_search_functionality() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index test files
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Indexing failed");

    // Test search by extension
    let pdf_results = fixture
        .repository
        .search_files_paginated(None, Some(".pdf"), 0, 10)
        .unwrap();
    assert_eq!(pdf_results.len(), 2);
    assert!(pdf_results.iter().all(|f| f.path.contains(".pdf")));

    // Test search by directory
    let doc_results = fixture
        .repository
        .search_files_paginated(None, Some("documents"), 0, 10)
        .unwrap();
    assert_eq!(doc_results.len(), 2);
    assert!(doc_results.iter().all(|f| f.path.contains("documents")));

    // Test search by filename
    let main_results = fixture
        .repository
        .search_files_paginated(None, Some("main"), 0, 10)
        .unwrap();
    assert_eq!(main_results.len(), 1);
    assert_eq!(main_results[0].path, "code/main.rs");

    // Test empty search returns all files
    let all_results = fixture
        .repository
        .search_files_paginated(None, Some(""), 0, 10)
        .unwrap();
    assert_eq!(all_results.len(), 4);
}

#[test]
fn test_selected_drive_basic_functionality() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index files on different drives
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Laptop indexing failed");

    fixture
        .repository
        .save("Work", "Desktop", 2048, &files)
        .expect("Desktop indexing failed");

    fixture
        .repository
        .save("Work", "Server", 4096, &files)
        .expect("Server indexing failed");

    // Test selecting specific drive
    let laptop_results = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 20)
        .unwrap();
    assert_eq!(laptop_results.len(), 4);
    assert!(laptop_results.iter().all(|f| f.drive_name == "Laptop"));

    let desktop_results = fixture
        .repository
        .search_files_paginated(Some("Desktop"), None, 0, 20)
        .unwrap();
    assert_eq!(desktop_results.len(), 4);
    assert!(desktop_results.iter().all(|f| f.drive_name == "Desktop"));

    let server_results = fixture
        .repository
        .search_files_paginated(Some("Server"), None, 0, 20)
        .unwrap();
    assert_eq!(server_results.len(), 4);
    assert!(server_results.iter().all(|f| f.drive_name == "Server"));

    // Test no drive selection (should return all)
    let all_results = fixture
        .repository
        .search_files_paginated(None, None, 0, 20)
        .unwrap();
    assert_eq!(all_results.len(), 12); // 4 files × 3 drives
}

#[test]
fn test_selected_drive_with_search_query() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index files on different drives
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Laptop indexing failed");

    fixture
        .repository
        .save("Work", "Desktop", 2048, &files)
        .expect("Desktop indexing failed");

    // Test combining drive selection with search query
    let laptop_pdf_results = fixture
        .repository
        .search_files_paginated(Some("Laptop"), Some(".pdf"), 0, 10)
        .unwrap();
    assert_eq!(laptop_pdf_results.len(), 2);
    assert!(
        laptop_pdf_results
            .iter()
            .all(|f| f.drive_name == "Laptop" && f.path.contains(".pdf"))
    );

    let desktop_pdf_results = fixture
        .repository
        .search_files_paginated(Some("Desktop"), Some(".pdf"), 0, 10)
        .unwrap();
    assert_eq!(desktop_pdf_results.len(), 2);
    assert!(
        desktop_pdf_results
            .iter()
            .all(|f| f.drive_name == "Desktop" && f.path.contains(".pdf"))
    );

    // Test search query without drive selection (should find PDFs on both drives)
    let all_pdf_results = fixture
        .repository
        .search_files_paginated(None, Some(".pdf"), 0, 10)
        .unwrap();
    assert_eq!(all_pdf_results.len(), 4); // 2 PDFs × 2 drives

    // Test drive selection with search query that has no matches
    let laptop_nonexistent_results = fixture
        .repository
        .search_files_paginated(Some("Laptop"), Some("nonexistent"), 0, 10)
        .unwrap();
    assert_eq!(laptop_nonexistent_results.len(), 0);
}

#[test]
fn test_selected_drive_nonexistent_drive() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index files on one drive
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Indexing failed");

    // Test selecting a nonexistent drive
    let nonexistent_results = fixture
        .repository
        .search_files_paginated(Some("NonexistentDrive"), None, 0, 10)
        .unwrap();
    assert_eq!(nonexistent_results.len(), 0);
    assert_eq!(nonexistent_results.len(), 0);

    // Test selecting a nonexistent drive with search query
    let nonexistent_with_query = fixture
        .repository
        .search_files_paginated(Some("NonexistentDrive"), Some(".pdf"), 0, 10)
        .unwrap();
    assert_eq!(nonexistent_with_query.len(), 0);
    assert_eq!(nonexistent_with_query.len(), 0);
}

#[test]
fn test_selected_drive_with_pagination() {
    let fixture = TestFixture::new();

    // Create many files for pagination testing
    let mut many_files = Vec::new();
    for i in 0..150 {
        many_files.push(FileEntry {
            path: format!("file_{i:03}.txt"),
            size_bytes: i * 10,
        });
    }

    // Index files on multiple drives
    fixture
        .repository
        .save("Test", "Drive1", 1024, &many_files)
        .expect("Drive1 indexing failed");

    fixture
        .repository
        .save("Test", "Drive2", 2048, &many_files)
        .expect("Drive2 indexing failed");

    // Test pagination with drive selection
    let drive1_page0 = fixture
        .repository
        .search_files_paginated(Some("Drive1"), None, 0, 100)
        .unwrap();
    assert_eq!(drive1_page0.len(), 100);

    let count = fixture
        .repository
        .count_search_results(Some("Drive1"), None)
        .unwrap();

    assert_eq!(count, 150);
    assert!(drive1_page0.iter().all(|f| f.drive_name == "Drive1"));

    let drive1_page1 = fixture
        .repository
        .search_files_paginated(Some("Drive1"), None, 1, 100)
        .unwrap();
    assert_eq!(drive1_page1.len(), 50);

    let count = fixture
        .repository
        .count_search_results(Some("Drive1"), None)
        .unwrap();

    assert_eq!(count, 150);
    assert!(drive1_page1.iter().all(|f| f.drive_name == "Drive1"));

    // Test pagination without drive selection (should see all files)
    let all_page0 = fixture
        .repository
        .search_files_paginated(None, None, 0, 100)
        .unwrap();
    assert_eq!(all_page0.len(), 100);

    let count = fixture.repository.count_search_results(None, None).unwrap();

    assert_eq!(count, 300); // 150 files × 2 drives

    let all_page2 = fixture
        .repository
        .search_files_paginated(None, None, 2, 100)
        .unwrap();
    assert_eq!(all_page2.len(), 100);

    let count = fixture.repository.count_search_results(None, None).unwrap();

    assert_eq!(count, 300);
}

#[test]
fn test_pagination_behavior() {
    let fixture = TestFixture::new();

    // Create many files to test pagination
    let mut many_files = Vec::new();
    for i in 0..250 {
        many_files.push(FileEntry {
            path: format!("file_{i:03}.txt"),
            size_bytes: i * 10,
        });
    }

    fixture
        .repository
        .save("Test", "Drive", 2048, &many_files)
        .expect("Indexing failed");

    // Test first page
    let page_0 = fixture
        .repository
        .search_files_paginated(None, None, 0, 100)
        .unwrap();
    assert_eq!(page_0.len(), 100);

    // Test second page
    let page_1 = fixture
        .repository
        .search_files_paginated(None, None, 1, 100)
        .unwrap();
    assert_eq!(page_1.len(), 100);

    // Test last page
    let page_2 = fixture
        .repository
        .search_files_paginated(None, None, 2, 100)
        .unwrap();
    assert_eq!(page_2.len(), 50);

    // Test beyond last page
    let page_3 = fixture
        .repository
        .search_files_paginated(None, None, 3, 100)
        .unwrap();
    assert_eq!(page_3.len(), 0);

    let count = fixture.repository.count_search_results(None, None).unwrap();

    assert_eq!(
        count,
        (page_0.len() + page_1.len() + page_2.len() + page_3.len()) as u64
    );
}

#[test]
fn test_search_result_count_accuracy() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("Indexing failed");

    // Test that search results len() is accurate
    let search_query = ".pdf";
    let search_results = fixture
        .repository
        .search_files_paginated(None, Some(search_query), 0, 100)
        .unwrap();

    // Should find exactly 2 PDF files
    assert_eq!(search_results.len(), 2);
    assert_eq!(search_results.len(), 2);

    // Verify the count matches the actual items returned
    assert!(search_results.iter().all(|f| f.path.contains(".pdf")));
}

#[test]
fn test_language_management_workflow() {
    let fixture = TestFixture::new();

    // Test default language
    let default_lang = fixture.repository.get_language().unwrap();
    assert_eq!(default_lang, Language::English);

    // Test language change
    let french = Language::French;
    fixture
        .repository
        .set_language(&french)
        .expect("Failed to set language");

    let current_lang = fixture.repository.get_language().unwrap();
    assert_eq!(current_lang, Language::French);

    // Test translation loading
    let translations = load_translations(&current_lang).unwrap();
    assert!(!translations.is_empty());

    // Test language toggle
    let toggled = current_lang.toggle();
    assert_eq!(toggled, Language::English);
}

#[test]
fn test_multiple_categories_and_drives() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index files in different categories and drives
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .expect("First indexing failed");

    fixture
        .repository
        .save("Personal", "Desktop", 512, &files)
        .expect("Second indexing failed");

    fixture
        .repository
        .save("Work", "Server", 8192, &files)
        .expect("Third indexing failed");

    // Verify all files are indexed
    let all_files = fixture
        .repository
        .search_files_paginated(None, None, 0, 20)
        .unwrap();
    assert_eq!(all_files.len(), 12); // 4 files × 3 locations

    // Verify different categories exist
    let categories: HashSet<_> = all_files.iter().map(|f| f.category_name.clone()).collect();
    assert!(categories.contains("Work"));
    assert!(categories.contains("Personal"));

    // Verify different drives exist
    let drives: HashSet<_> = all_files.iter().map(|f| f.drive_name.clone()).collect();
    assert!(drives.contains("Laptop"));
    assert!(drives.contains("Desktop"));
    assert!(drives.contains("Server"));
}

#[test]
fn test_edge_cases_and_error_handling() {
    let fixture = TestFixture::new();

    // Test empty search
    let empty_result = fixture
        .repository
        .search_files_paginated(None, Some(""), 0, 10)
        .unwrap();
    assert_eq!(empty_result.len(), 0);

    // Test search with no matches
    let no_matches = fixture
        .repository
        .search_files_paginated(None, Some("nonexistent"), 0, 10)
        .unwrap();
    assert_eq!(no_matches.len(), 0);

    // Test pagination with no data
    let no_data = fixture
        .repository
        .search_files_paginated(None, None, 5, 10)
        .unwrap();
    assert_eq!(no_data.len(), 0);
    assert_eq!(no_data.len(), 0);

    // Test remove duplicates with no data
    let remove_empty = fixture.repository.remove_duplicates("NonExistent", "Drive");
    assert!(remove_empty.is_ok());

    // Test indexing empty file list
    let empty_index = fixture.repository.save("Empty", "Drive", 0, &[]);
    assert!(empty_index.is_ok());
    assert_eq!(empty_index.unwrap(), 0);

    // Test selected drive edge cases
    let empty_drive_name = fixture
        .repository
        .search_files_paginated(Some(""), None, 0, 10)
        .unwrap();
    assert_eq!(empty_drive_name.len(), 0);
}

// Benchmark test to ensure performance doesn't regress
#[test]
fn test_search_performance_with_large_dataset() {
    let fixture = TestFixture::new();

    // Create a large dataset
    let mut large_dataset = Vec::new();
    for i in 0..10000 {
        large_dataset.push(FileEntry {
            path: format!(
                "category_{}/subcategory_{}/file_{:05}.txt",
                i % 10,
                i % 100,
                i
            ),
            size_bytes: i,
        });
    }

    fixture
        .repository
        .save("Large", "Dataset", 16384, &large_dataset)
        .expect("Large dataset indexing failed");

    let start = Instant::now();

    // Test search performance
    let search_result = fixture
        .repository
        .search_files_paginated(None, Some("category_5"), 0, 100)
        .unwrap();

    let elapsed = start.elapsed();

    // Ensure reasonable performance (adjust threshold as needed)
    assert!(elapsed.as_millis() < 1000);
    assert!(!search_result.is_empty());

    // Test pagination performance
    let start = Instant::now();
    let page_result = fixture
        .repository
        .search_files_paginated(None, None, 50, 100)
        .unwrap();
    let elapsed = start.elapsed();

    assert!(elapsed.as_millis() < 500);
    assert_eq!(page_result.len(), 100);
}

#[test]
fn test_delete_entire_drive() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .unwrap();

    fixture.repository.delete("Laptop", None).unwrap();

    let result = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 10)
        .unwrap();
    assert!(result.is_empty());

    let count = fixture
        .repository
        .count_search_results(Some("Laptop"), None)
        .unwrap();
    assert_eq!(count, 0);
}

#[test]
fn test_delete_specific_category() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .unwrap();
    fixture
        .repository
        .save("Personal", "Laptop", 1024, &files)
        .unwrap();

    let category = Some("Work");
    fixture.repository.delete("Laptop", category).unwrap();

    let result = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 10)
        .unwrap();
    assert!(result.iter().all(|f| f.category_name != "Work"));
    assert!(result.iter().any(|f| f.category_name == "Personal"));
    let count = fixture
        .repository
        .count_search_results(Some("Laptop"), None)
        .unwrap();
    assert_eq!(count, files.len() as u64); // only Personal category files remain
}

#[test]
fn test_delete_nonexistent_drive_or_category() {
    let fixture = TestFixture::new();
    fixture.repository.delete("NonexistentDrive", None).unwrap();
    let category = Some("NonexistentCategory");
    fixture.repository.delete("Laptop", category).unwrap();
}

#[test]
fn test_delete_entire_drive_removes_stale_categories() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Insert files in two categories
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .unwrap();
    fixture
        .repository
        .save("Personal", "Laptop", 1024, &files)
        .unwrap();

    // Make sure categories exist
    let categories_before: Vec<String> = fixture
        .repository
        .find_all_category_names_for_drive("Laptop")
        .unwrap();
    assert!(categories_before.contains(&"Work".to_string()));
    assert!(categories_before.contains(&"Personal".to_string()));

    // Delete the entire drive
    fixture.repository.delete("Laptop", None).unwrap();

    // Drive files should be gone
    let result = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 10)
        .unwrap();
    assert!(result.is_empty());

    // Stale categories should be removed
    let categories_after: Vec<String> = fixture
        .repository
        .find_all_category_names_for_drive("Laptop")
        .unwrap();
    assert!(!categories_after.contains(&"Work".to_string()));
    assert!(!categories_after.contains(&"Personal".to_string()));
}

#[test]
fn test_delete_specific_category_removes_stale_category() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Insert files in two categories
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .unwrap();
    fixture
        .repository
        .save("Personal", "Laptop", 1024, &files)
        .unwrap();

    // Delete a specific category
    let category = Some("Work");
    fixture.repository.delete("Laptop", category).unwrap();

    // Work files should be gone, Personal files remain
    let remaining_files = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 10)
        .unwrap();
    assert!(remaining_files.iter().all(|f| f.category_name != "Work"));
    assert!(
        remaining_files
            .iter()
            .any(|f| f.category_name == "Personal")
    );

    // Stale category 'Work' should be removed
    let categories_after: Vec<String> = fixture
        .repository
        .find_all_category_names_for_drive("Laptop")
        .unwrap();
    assert!(!categories_after.contains(&"Work".to_string()));
    assert!(categories_after.contains(&"Personal".to_string()));
}

#[test]
fn test_category_not_deleted_if_used_by_another_drive() {
    let fixture = TestFixture::new();
    let files = TestFixture::create_test_files();

    // Index the same category "Work" on two drives
    fixture
        .repository
        .save("Work", "Laptop", 1024, &files)
        .unwrap();
    fixture
        .repository
        .save("Work", "Desktop", 2048, &files)
        .unwrap();

    // Delete all files on Laptop
    fixture.repository.delete("Laptop", None).unwrap();

    // Verify Laptop has no files
    let laptop_files = fixture
        .repository
        .search_files_paginated(Some("Laptop"), None, 0, 10)
        .unwrap();
    assert!(laptop_files.is_empty());

    // Verify category "Work" still exists because Desktop uses it
    let desktop_categories = fixture
        .repository
        .find_all_category_names_for_drive("Desktop")
        .unwrap();
    assert!(desktop_categories.contains(&"Work".to_string()));

    // Verify "Work" is not listed as stale globally
    let laptop_categories = fixture
        .repository
        .find_all_category_names_for_drive("Laptop")
        .unwrap();
    assert!(!laptop_categories.contains(&"Work".to_string()));
}
